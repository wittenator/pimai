...
def build_dataset(path, prob):
    ...
    mnist_train_occluded = datasets.MNIST(f'{path}data', train=True, transform=transforms.Compose([transforms.ToTensor()]))
    mnist_train_occluded.targets[train_occluded] = -1
    mnist_test_occluded = datasets.MNIST(f'{path}data', train=False, transform=transforms.Compose([transforms.ToTensor()]))
